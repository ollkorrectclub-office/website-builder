...
function mapWorkspaceMemberEventRow(row: Record<string, unknown>): WorkspaceMemberEventRecord {
  return {
    ...
    previousOwnerUserId: row.previous_owner_user_id ? String(row.previous_owner_user_id) : null,
    previousOwnerName: row.previous_owner_name ? String(row.previous_owner_name) : null,
    previousOwnerEmail: row.previous_owner_email ? String(row.previous_owner_email) : null,
    nextOwnerUserId: row.next_owner_user_id ? String(row.next_owner_user_id) : null,
    nextOwnerName: row.next_owner_name ? String(row.next_owner_name) : null,
    nextOwnerEmail: row.next_owner_email ? String(row.next_owner_email) : null,
...
async function appendWorkspaceMemberEventSupabase(event: WorkspaceMemberEventRecord) {
  ...
  const { error } = await client.from("workspace_member_events").insert({
    ...
    previous_owner_name: event.previousOwnerName,
    previous_owner_email: event.previousOwnerEmail,
    next_owner_user_id: event.nextOwnerUserId,
    next_owner_name: event.nextOwnerName,
    next_owner_email: event.nextOwnerEmail,
